...
// Activity for configuring a password with 3 non consecutive cells.
public class S3NCCellsActivity extends Activity {

	private int[][] tab = new int[9][9];
	private TextView textview_activity_s3nccells_instructions;
	private Button button_activity_s3nccells_exit;
	private Spinner abs1, ord1, abs2, ord2, abs3, ord3; // Spinners to get the coordinates.
	private Button button_activity_s3nccells_submit;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.activity_s3nccells);
		...
		button_activity_s3nccells_exit = (Button) findViewById(R.id.button_activity_s3nccells_exit);
		button_activity_s3nccells_exit.setEnabled(false);
		button_activity_s3nccells_exit.setOnClickListener(new Button.OnClickListener() {
	...
	public void onStart() {
		...
		button_activity_s3nccells_submit = (Button) findViewById(R.id.button_activity_s3nccells_submit);
		// / On définit le listener du bouton valider.
		button_activity_s3nccells_submit.setOnClickListener(new Button.OnClickListener() {
			public void onClick(View v) {
				...
				password.set_type(1);	// Password with 3 non consecutive cells (NCC), then the type variable is equals to "1".

				if (!(((_abs1 == _abs2) && (_ord1 == _ord2)) || ((_abs1 == _abs3) && (_ord1 == _ord3)) || ((_abs2 == _abs3) && (_ord2 == _ord3)))) {
					...
					if (manager.save(password, context)) {
						Toast.makeText(S3NCCellsActivity.this, "Preferences saved", Toast.LENGTH_SHORT).show();
						button_activity_s3nccells_exit = (Button) findViewById(R.id.button_activity_s3nccells_exit);
						button_activity_s3nccells_exit.setEnabled(true);
					}
				}
				// Else, the user made bad choices (at least one cell chosen several times.)
				else {
					Toast.makeText(S3NCCellsActivity.this, "Wrong coordinates", Toast.LENGTH_SHORT).show();
				}
			}
		});
	}
	...
	public void tableGeneration() {
		...
		textview_activity_s3nccells_instructions = (TextView) findViewById(R.id.textview_activity_s3nccells_instructions);
		textview_activity_s3nccells_instructions.setText("The password will be the content of the cells choosen according to the order.\nFor example if you choose as first cell A1, A9 and finally I9, the password will be (with this randomly generated table): " + tab[0][0] + tab[8][0] + tab[8][8] + ".\nNote: the cells have to be different from each others.");
	}
}
